...
- PROP READS COME FROM THE PROP BOARD. A +EV prop call is a devigged prop-board edge (the prop board / the prop plays) — that's the only thing that earns a "we like this prop" read. A raw consensus prop line (a plain over/under number with no board edge) is CONTEXT, not a play: quote it as context if asked, but never dress a consensus line up as a board edge or a +EV claim. No board edge, no prop play — same discipline as the 6% floor on games.
