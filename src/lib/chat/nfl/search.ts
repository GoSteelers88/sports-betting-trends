...
export const SEARCH_ALLOWED_DOMAINS: readonly string[] = [
  // News / reference — schedules, injuries, transactions, box scores.
  "nfl.com",
  "espn.com",
  "cbssports.com",
  "nbcsports.com",
  ...
  "yahoo.com",
  // Line carriers, added 2026-09-09. There is no live NFL prop odds feed in
  // this repo, so without a source that PUBLISHES prop numbers the desk could
  // only ever say "here's the number I'd need" — which is what the operator
  // rejected. These carry posted player-prop lines and game odds; a line taken
  // from one is attributed to it, never spoken in the desk's own voice.
  "rotowire.com",
  "oddsshark.com",
  "covers.com",
  "actionnetwork.com",
...
/** Five searches. Three covered "who plays Sunday" and "is he practicing", but
 *  a prop question needs the posted line AND the usage AND the injury status,
 *  which is three on its own before any cross-check. `max_uses_exceeded` comes back as
...
// ⚠️ BASIC variant on purpose. `web_search_20260209` adds dynamic filtering,
// and that filtering RUNS CODE EXECUTION under the hood. In a multi-turn tool
// loop — which is exactly what receipts mode is, board + market + injuries then
// search — the next request fails with:
//
...
//
// Measured 2026-09-09 on "give me some player props": the turn 500'd and the
// user got "Something glitched on my end." Threading a container id through
// every loop iteration is the alternative; the basic variant needs no container
// and supports the two things this feature actually depends on — max_uses and
// allowed_domains. Dynamic filtering buys nothing here because the allowlist
