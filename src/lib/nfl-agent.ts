...
function pickSystemPrompt(): string {
  return [
    ...
    "     look-ahead). The injury text may carry the source status (e.g.",
    "     'Knee — Injured Reserve (est. return 2026-10-11)'): a season-long IR",
    "     player is ALREADY in the market's price — a fresh Out/Doubtful is not.",
    "     An empty list means no reported injuries for that team; if BOTH lists",
    "     are empty for every game on the slate, treat injuries as UNKNOWN, not",
    "     absent, and say so — never write 'no injuries' from silence.",
    ...
    "     context.neutralSite=true means a NEUTRAL venue (international series /",
    "     Super Bowl): there is NO home field — both teams travelled, the listed",
    "     'home' team gets no crowd or travel credit, and the market's typical",
    "     ~2.5-point home edge should be absent from your read.",
    ...
    "     run. Cold/precip dampens scoring. Dome games negate weather. On a live",
    "     week temp/wind are a kickoff FORECAST (context.weatherForecastHoursAhead",
    "     = the horizon); a 5-day-out wind number is a lean, not a fact. If temp",
    "     and wind are null for an OUTDOOR game the forecast was unavailable —",
    "     treat weather as UNKNOWN, never as calm. A blank roof is retractable:",
    "     it may be closed at kickoff.",
    ...
    " 13. Efficiency (context.epa, when present) — EWMA EPA per play for each",
    "     side over its last ~20 games, weighted by plays, computed only from",
    "     games before this week: offPassEpaPerDropback / offRushEpaPerCarry",
    "     (offense; higher = better) and defPassEpaPerDropbackAllowed /",
    "     defRushEpaPerCarryAllowed (defense; LOWER = better). Scale: ±0.10 per",
    "     dropback is elite/awful; ±0.03 is ordinary. This is the strength",
    "     signal — prefer it to W-L and point differential, which it explains.",
    "     A matchup edge is offense EPA vs the opposing defense's allowed EPA in",
    "     the SAME phase (pass vs pass, rush vs rush). gamesInWindow tells you",
    "     how much is prior-season carry-over (down-weighted). null = no data.",
    "",
    ...
    "confidence IS your probability that moneylineSide wins outright. Use the",
    "full 0.50–0.80 range at 0.01 resolution and make it game-specific: start",
    "from the moneyline's implied probability, then move it by the factors you",
    "can NAME (EPA gap, QB status, neutral site, wind). Six games at exactly",
    "0.55 is a template, not six reads — the calibration record shows the model",
    "has been UNDER-confident on moneylines it was right about.",
    "",
