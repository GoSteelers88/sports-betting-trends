...
async function main() {
  ...
  // Load MLB model output (generated by ingest:mlb-model, runs before ingest:free in ingest:all)
  type MlbModelFile = { generatedAt: string; results: unknown[] };
  const mlbModelData = loadJson<MlbModelFile>(path.join(processedDir, "mlb-model-output.json"));
  const mlbModel = mlbModelData?.results ? mlbModelData : undefined;
